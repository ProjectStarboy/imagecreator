...
export class ScreenshotService extends AppService {
  ...
  takeScreenshot(playerSrc: string, name: string, bucket: string) {
  ...
  async processImage(
    ...
    for (let w = 0; w < width; w++) {
      for (let h = 0; h < height; h++) {
        ...
        const maskedInt = Jimp.rgbaToInt(
          rgbaColor.r,
          rgbaColor.g,
          rgbaColor.b,
          rgbaColor.a
    ...
    return new Promise<string>((resolve, reject) => {
      image.getBuffer('image/png', async (err, buffer) => {
        const { data: hasBucket } = await supabase.storage.getBucket(bucket)
        if (!hasBucket) {
          const { data: createBucketData, error } =
        ...
        if (data) {
          const publicPathResponse = await supabase.storage
            .from(bucket)
